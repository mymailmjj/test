...
public class BTree<K extends Comparable<K>> {
	 ...
	 * @param length
	 */
	private void splitCopyNode(BTree<K>.Node<K> sourceNode, BTree<K>.Node<K> targetNode, int fromIndexSrc,
	 ...
	 * @param parentNode
	 */
	private void resetChildNode(BTree<K>.Node<K> parentNode) {
	 ...
	 * @param node
	 */
	private void initNode(BTree<K>.Node<K> node) {
		node.keys = new Object[half_pahse];
		node.values = new Object[half_pahse];
	 ...
	 * @param node
	 */
	private void split(BTree<K>.Node<K> node) {
		...
		if (node.isLeaf) {
			splitCopyNode(node, rightNode, m+1, 0, node.keys.length - m-1);
		} else {
			splitCopyNode(node, rightNode, m+1, 0, node.keys.length - m-1);
		}
		...
		// 不是空，则递归调用，切分方法
			...
			if (countSize == half_pahse && keys.length == half_pahse) {
				// 扩容
	 ...
	 *            插入的value
	 */
	private void insert(BTree<K>.Node<K> node, K key, Object e) {
	 ...
	 * @param capacity
	 */
	private void ensureCapacity(BTree<K>.Node<K> node, int capacity) {
		if (node == null)
